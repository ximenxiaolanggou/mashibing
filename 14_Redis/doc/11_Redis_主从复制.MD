...
### 在Redis解压文件中utils目录下执行 ./install_server.sh

1. 创建6379

   ```shell
   [root@LocalCentOS7 utils]# ./install_server.sh 
   ...
   Please select the redis executable path [/usr/local/bin/redis-server] redis-server-6379
   Selected config:
   Port           : 6379
   ...
   Copied /tmp/6379.conf => /etc/init.d/redis_6380
   Installing service...
...
2. 创建6380
   ...
   Please select the redis executable path [/usr/local/bin/redis-server] redis-server-6380
   Selected config:
   Port           : 6380
...
3. 创建6381
   ...
   Please select the redis port for this instance: [6381] 6381
   ...
   Please select the redis executable path [/usr/local/bin/redis-server] redis-server-6381
   Selected config:
   Port           : 6381
...
### 将三个Redis实例设置为非守护进程启动并关掉日志(关掉日志会将日志打印在控制台便于观察信息)

> 修改Redis配置文件 6379.conf、6380.conf、6381.conf
...
OK
# 设置主库的用户名（如果主库有）
127.0.0.1:6379> CONFIG SET masteruser username
ok
# 配置主库地址
...
**可以在redis服务启动时指定master**

```shell
redis-server ./6380.conf --replicaof 127.0.0.1 6378

redis-server ./6381.conf --replicaof 127.0.0.1 6378
```
...
slave_repl_offset:121
slave_priority:100
slave_read_only:1
connected_slaves:0
master_replid:2a93052ee8b89684edc4f649419eed1159645bfd
master_replid2:0000000000000000000000000000000000000000
master_repl_offset:121
second_repl_offset:-1
repl_backlog_active:1
repl_backlog_size:1048576
repl_backlog_first_byte_offset:1
repl_backlog_histlen:121
127.0.0.1:6380> 
```
...
- **主库**宕机：从库原地待命，仅读取，不可写入。

- **主库**恢复：从库自动重新连接。

- **从库**宕机：仅宕机的从库不可访问，其他正常。
...
- **主库**宕机：从库原地待命，仅读取，不可写入。主库恢复后从库自动重新连接
- **从库**宕机：中间从库宕机，后面的从库均不会有新数据产生。

### 手动解决宕机问题

当一个主库宕机后，后面的从库执行`REPLICAOF no one`可以升为主库，**一主多从**模式下其他的从库需要执行`REPLICAOF host port`更换主库，**薪火相传**模式后面的从库不用做任何修改。
...
1. replica-serve-stale-data yes
   ...
   ```shell
   当salve启动时连接master，但是master中的数据量可能会很大，所以在数据同步时需不需要将salve已存在的数据对外暴露
   如果设置为yes代表同步完才对外提供
   ```
...
4. repl-backlog-size 1mb

   ```shell
   当slave挂掉，过段时间继续运行时，并且这段时间master产生的数据够存放在队列中，就会使用偏移量offset进行写入
   如果master产生的数据量较快，设置的队列大小不够，就会触发全量同步
   ```
...
5. min-replcas-to-write 3、min-replcas-max-lag 10

   ```shell
   最小几个写成功，默认是注释掉的，因为这就往强一致性上靠了，所以根据用户实际情况自行选择
   ```
